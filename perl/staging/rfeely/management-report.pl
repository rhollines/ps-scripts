...
management-report.pl


=head1 SYNOPSIS

management-report.pl [options] --project PROJECT --to USERS
...
Report on new and fixed defects appearing in PROJECT

=item B<--to>

CIM USERS, comma separated, enclosed in double quotes
...
Send an email to explicit CIM users reporting on New defects appearing 
in a project and Fixed defects within the last N days.  This script is 
not designed to operate explicitly on streams.
...
foreach my $md (@defects) {
  my $fD_iso_8601 = $md->{firstDetected};
  #print "stored value: $fD_iso_8601\n";

  my $fD_sec_since_epoch = parse_timestamp($fD_iso_8601);
  #print "first detected\n";
  #print_timestamp($sec_since_epoch);

  if ($fD_sec_since_epoch > $cutoff) {
    push @new_defects, $md;
  }

  if (($md->{status} eq "Fixed") and (exists $md->{lastFixed})) {
    my $lF_iso_8601 = $md->{lastFixed};
    my $lF_sec_since_epoch = parse_timestamp($lF_iso_8601);
    if ($lF_sec_since_epoch > $cutoff) {
      push @fixed_defects, $md;
    }
  }
}
...
my $fixed_defect_count = @fixed_defects;
print "  of which $new_defect_count are new and $fixed_defect_count have been fixed since the cutoff time of $opt_days day(s)\n";
...
if ($new_defect_count == 0) {
  $body .= "No defects were";
} elsif ($new_defect_count == 1) {
  $body .= "The following defect was";
} else {
  $body .= "The following $new_defect_count defects were";
}

$body .= " found in project $opt_project within the past ";

if ($opt_days == 1) {
  $subject = "New and fixed defects detected in the past 24 hours by Coverity";
  $body .= "24 hours.</p>";
} else {
  $subject = "New and fixed defects detected in the past $opt_days days by Coverity";
  $body .= "$opt_days days.</p>";
}

$body .= "<br/><table border=\"1\"><tr><th>CID</th><th>Checker</th><th>Classification</th><th>Owner</th><th>Severity</th><th>Action</th><th>Function</th><th>File</th><th>Build #</th></tr>";
foreach my $md (@new_defects) {
  my $url = "http://$opt_host:$opt_port/sourcebrowser.htm?projectId=$projectId#mergedDefectId=$md->{cid}";
  $body .= "<tr><td><a href=\"$url\">$md->{cid}</a></td><td>$md->{checker}</td><td>$md->{classification}</td><td>$md->{owner}</td><td>$md->{severity}</td><td>$md->{action}</td><td>$md->{functionDisplayName}</td><td>$md->{filePathname}</td><td>$md->{externalReference}</td></tr>";
}
...
if ($fixed_defect_count == 0) {
  $body .= "No defects were";
} elsif ($fixed_defect_count == 1) {
  $body .= "The following defect was";
} else {
  $body .= "The following $fixed_defect_count defects were";
}

$body .= " fixed in project $opt_project within the past ";
