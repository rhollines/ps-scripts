...
package Coverity::WS::v1::Service::CovRemoteServiceException;
...
package Coverity::WS::v1::Service;

our $VERSION = 0.2;
our $VERSION_STRING = "v1";
...
sub ws_authen_nonce {
  ...
  $auth->value( \SOAP::Data->value(
    _complex_type ( "wsse:UsernameToken",
      ...
      _typeless("wsse:Password",$passwordHash)->attr({
...
sub new {
  ...
  } elsif ($self->{authtype} =~ /digest/i) {
    $self->{wss} = ws_authen_nonce($param{username}, $param{password},
      \default_nonce_generator());
  } else {
  ...
  $self->{soap_proxy}->serializer(Coverity::WS::v1::Serializer->new);
...
sub AUTOLOAD {
  ...
  if ($self->{service_methods}->{$method}) {
    ...
    if (!$som->fault()) {
      ...
      my $exception = new Coverity::WS::v1::Service::CovRemoteServiceException(
    ...
    die new Coverity::WS::v1::Service::CovRemoteServiceException(
      errorCode => -1,
...
Coverity::WS::v1::Service - Base class for all Coverity Integrity Manager Web Services

=head1 DESCRIPTION

Coverity::WS::v1::Service is an abstract base class for all CIM Web Services.
...
A service-specific subclass first inherits from Coverity::WS::v1::Service:

  our @ISA = qw(Coverity::WS::v1::Service);
...
Our service may also require some complex data types, which all in herit from DataObj:
  ...
  package Coverity::WS::v1::ProjectService::streamSpecDataObj;

  # Inherit from DataObj base class
  use Coverity::WS::v1::DataObj
    'description' => ["pod", 0 ],
    'language' => ["pod", 0 ],
    'name' => ["pod", 0 ],
    'type' => ["pod", 0 ];

  our @ISA = qw(Coverity::WS::v1::DataObj);

That's it, we now have a service we can use!

  use Coverity::WS::v1::ProjectService;

  my $projectService = new Coverity::WS::v1::ProjectService(
                          host => "localhost",
                          username => "admin",
                          password => "coverity"
                        );
